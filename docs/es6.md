...
ネット上で`ES6`と`ES2015`を見かけると思いますが、両方とも同じものを指しています。正確には`ES2015`が正しいです。来年以降も`ES2016`,`ES2017`…とアップデート予定です。当初は`ES6`として仕様検討が始まったためその名残りで残っています。~~この資料では~~`ES6`~~で統一させていただきます。\(短いほうが読みやすいので\)~~
...
ブラウザに新仕様が追加されるまで新仕様を導入することがきません。そこで登場したのが`Babel(6to5)`で、`Babel`は`ES2015,ES Next`のコードを`ES5`のコードにトランスパイルしてくれるツールです\([公式サイト](https://babeljs.io/)\)。`ES2016`,`ES2017`の様な次期仕様を`ES Next`と呼んでいます。
